...
## Phase 5: Debate Room — Multi-Agent Deliberation (Week 29-34)

> "NexusCall 킬러 피처 — 에이전트들이 토론하여 더 나은 결론을 도출하는 구조화된 토론방"

### 핵심 기능
1. **Debate Room 생성** — 주제, 참여 에이전트, 라운드 수, 모드 설정
2. **구조화된 토론** — 강점 인정 → 허점 지적 → 대안 제시 프롬프트 구조
3. **실시간 스트리밍** — WebSocket으로 라운드별 결과 실시간 전달
4. **자동 합성** — 모든 라운드 완료 후 최종 합의안 자동 생성
5. **Devil's Advocate 모드** — 일부러 반대 입장 강제하여 Echo Chamber 방지
6. **합의 속도 감지** — 너무 빠른 합의 시 자동 라운드 추가

### API Endpoints
- `POST /api/debates` — Debate Room 생성
- `GET /api/debates` — 목록 조회
- `GET /api/debates/:id` — 상세 (라운드 결과 포함)
- `POST /api/debates/:id/start` — 토론 시작
- `POST /api/debates/:id/rounds` — 수동 라운드 추가
- `GET /api/debates/:id/synthesis` — 최종 합성 결과
- `WebSocket /api/debates/:id/stream` — 실시간 토론 스트리밍

### DB Tables
- `debates` — 토론방 메타데이터
- `debate_rounds` — 라운드별 각 에이전트의 응답
- `debate_syntheses` — 최종 합성 결과
...
Week 29-34  ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░████████████  Phase 5: Debate Room

MVP Ship: End of Week 8 (Phase 1 + Phase 2 core)
Beta: End of Week 18
GA: End of Week 28
Debate Room: End of Week 34
```
